...
<Modal class="*:first:max-w-3xl" bind:open>
	<div class="max-h-[70vh]">
		<h3 class="ml-2 card-title">{m.D_SM_CONNECTION__S_TITLE({ input0: location.host })}</h3>
						...
						<td>
							...
							<div class="tooltip" data-tip={m.D_SM_CONNECTION__IDOK()}>
								<button
									...
									class="btn btn-square btn-soft btn-xs btn-info">
									<Info />
								</button>
							</div>
						</td>
...
{#if selectedDetail != undefined && !infoDetail.isError}
	<Modal class="*:first:max-w-3xl" open onclose={() => (selectedDetail = undefined)}>
		<div class="max-h-[70vh]">
			<h3 class="ml-2 card-title">{m.SM_CONNINFO_CAPTION({ input0: infoDetail.data.Name_str })}</h3>
			<table class="table-pin-rows table mt-2 table-zebra table-sm">
				<thead>
					<tr>
