<script lang="ts">
	...
	const query = createQuery(() => ({
		...
		queryFn: () => rpc.GetHubExtOptions(new VpnRpcAdminOption({ HubName_str: hub })),
		enabled: open,
		initialData: new VpnRpcAdminOption()
	}));

	const mutation = createMutation(() => ({
		mutationFn: rpc.SetHubExtOptions,
		onSuccess: () => {
...
<Modal {id} class="*:first:max-w-2xl" bind:open>
	<div class="mb-3 flex items-center gap-2">
		...
		<h3 class="text-lg font-semibold">{m.SM_HUBEXT_OPTION_TITLE()}</h3>
		{#if query.isFetching}
	...
	<div class="my-4 alert alert-soft alert-info">
		<InfoIcon size={32} />
		<div>
			<p>{m.SM_HUBEXT_OPTION_STATIC1()}</p>
			<p>{m.SM_HUBEXT_OPTION_STATIC2()}</p>
		</div>
	</div>
